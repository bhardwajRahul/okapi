...
Two entry points are provided, both taking an optional `WebConfig`:

- `Web(prefix, dir, cfg...)` — serve from a directory on disk.
- `WebFS(prefix, fsys, cfg...)` — serve from any `fs.FS`, including an `embed.FS`
...
`WebFS` checks at registration that the index document exists inside the
configured `Root`, and logs an error if it does not — the app would otherwise
answer `404` for every client-side route.

## How routing works

The handler is registered for `GET` and `HEAD` under the prefix. For every
request it does not exclude:
...
`/api/v1/missing` returns `404` rather than `index.html`. An excluded path is
answered by your `NoRoute` handler if you have set one, and by the standard
`404` otherwise.
...
| `Root` | Sub-directory inside the `fs.FS` that holds the built app (`WebFS` only; ignored by `Web`). |
