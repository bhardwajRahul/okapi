...
Newlines are stripped from the `id` and `event` fields before they are written, so
a value derived from user input — a reflected `Last-Event-ID`, say — cannot forge
extra fields or whole events. In the data payload, a lone `\r` is treated as a
line end, as browsers do, so relayed text cannot inject `id:`/`event:` lines or
split an event either.
...
`SSEStream` returns when the channel is closed, when the request context is
cancelled (the client disconnected — `ctx.Err()` is returned then), or when the
server begins a graceful shutdown, in which case it returns `nil`. An error
returned after the stream has started is logged, not written to the client: the
response is long since committed.
...
`SSEStreamWithOptions` ends on the same three conditions as `SSEStream`, and a
`PingInterval` greater than zero sends a `: ping` comment line at that interval to
...
<body>
    ...
    <script>
        ...
        function addEvent(type, data) {
            ...
            eventElement.textContent = type + ': ' + data;
...
Select on the request context so the handler returns as soon as the client goes
away — and pace the loop with a ticker rather than spinning:
...
o.Get("/events", func(c *okapi.Context) error {
    ...
    for {
        ...
        case <-ctx.Done():
            ...
            return nil
        case <-ticker.C:
            if err := c.SSESendData("data"); err != nil {
                return err
            }
        }
    }
...
### 6. Expect Shutdown to End the Stream

A graceful shutdown closes every active stream: `SSEStream` and
`SSEStreamWithOptions` return `nil` as soon as shutdown begins, so a deploy does
not wait out your longest-lived connection. Producers should stop on
`ctx.Done()` — cancelled once draining finishes — and clients reconnect on their
own.
...
Bundled serializers: `okapi.JSONSerializer{}` (the default),
`okapi.TextSerializer{}` and `okapi.Base64Serializer{}`. Implement
`okapi.Serializer` for your own.
