...
`c.BaseURL()` returns the base URL, `c.Request(method, path)` builds a request for
any method, and `c.Do(ctx, req)` sends an `*http.Request` you built yourself
through the client's middleware and retry chain.
...
`Headers(map)` and `QueryParams(map)` set several at once, and `Path(p)` replaces
the path on a builder.
...
`Do` returns a `*Response` that embeds the underlying `*http.Response` with the body already read into memory:
...
resp.StatusCode              // int
resp.Method                  // the request's method
resp.URL                     // the final request URL
```

`Response.Body` is a `[]byte`, shadowing the embedded `http.Response.Body`: the
stream has already been read and closed for you.
...
When the last attempt still fails with a retryable status, that response is returned intact — its body has not been drained — so the caller still sees the status and error body.
...
`HTTPError` carries `StatusCode`, `Status`, `Method`, `URL`, `Body` and `Header`.
